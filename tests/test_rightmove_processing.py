...
def test_get_listings(rightmove_sample: BeautifulSoup) -> None:
    ...
    assert listings[0].description == "A fully furnished double bedroom is available for couples with balcony in a four-bedroom with one bathroom house-share. This property is conveniently situated in zone 2, providing excellent transportation connections to central London, with easy access to Upper Holloway station nearby."
    assert listings[0].price == "£1,127 pcm"
    assert listings[0].added_date == "10/02/2024"
    assert listings[0].phone_number == "020 4538 5940"
